...
async def handle_exception(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(context.error, KeyError):  # wildcard
        ...
        await update.message.reply_text(f'❌ В данный момент сайт недоступен. Повторите попытку позже.')
    else:
        await update.message.reply_text(f'❌ Произошла неизвестная ошибка:\n {str(context.error)}')
        logging.error("".join(traceback.format_exception(None, context.error, context.error.__traceback__)))
